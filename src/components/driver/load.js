...
import { Form, Button, Input, Label, Modal, Select, TextArea } from 'semantic-ui-react';
...
function Load({ load, cancelLoad, cancel }) {
    ...
    const renderCancelOrDelete = () => {
        if(cancel){
            return <Button color='red' onClick={cancelledLoad} style={{margin: "auto"}}>Cancel Load</Button>
        }
        return <Button color='red' onClick={handleDelete} style={{margin: "auto"}}>Delete Load</Button>
    }
    ...
    return(
        <div style={loadStatusBorderColor()}>
            {loadInfo.trailerType} {loadInfo.trailerNumber} 
            <br></br>
